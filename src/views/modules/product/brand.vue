    ...
    <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandle"
      ...
      <el-table-column prop="logo" header-align="center" align="center" label="品牌logo地址">
        <template slot-scope="scope">
          <img  v-if="scope.row.logo" :src="scope.row.logo" width="100px" height="100px"/>
      </template>
      </el-table-column>
      ...
      <el-table-column prop="showStatus" header-align="center" align="center" label="显示状态[0-不显示；1-显示]">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.showStatus" active-color="#13ce66" inactive-color="#ff4949"
            ...
            @change="updateShowDtatus(scope.row)">
          </el-switch>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    updateShowDtatus(data) {
      console.log(data)
      // 更新当前记录的显示状态
      this.$http({
        url: this.$http.adornUrl("/product/brand/update"),
        method: "post",
        data: this.$http.adornData({ "brandId": data.brandId, "showStatus": data.showStatus?1:0 }, false),
      }).then(({ data }) => {
        this.$message({
          message: "状态更新成功",
          type: "success",
